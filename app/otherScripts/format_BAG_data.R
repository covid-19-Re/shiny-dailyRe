...
incidence_onset <- sapply(allDates_onset, function(x) { sum(with(data_hospitalization, manifestation_dt == x & hospitalisation == 1), na.rm = T)})
...
partialInfoAdmissions <- subset(data_hospitalization, ymd(eingang_dt) %in% allDates_admission & !(ymd(hospdatin) %in% allDates_admission) & !(ymd(manifestation_dt) %in% allDates_onset) & hospitalisation == 1)
fullInfoAdmissions <- subset(data_hospitalization, ymd(hospdatin) %in% allDates_admission & !(ymd(manifestation_dt) %in% allDates_onset) & hospitalisation == 1)
...
partialInfoAdmissions <- subset(data_hospitalization, ymd(eingang_dt) %in% allDates & !(ymd(hospdatin) %in% allDates) & hospitalisation == 1)
fullInfoAdmissions <- subset(data_hospitalization, ymd(hospdatin) %in% allDates & hospitalisation == 1)

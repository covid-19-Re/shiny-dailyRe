...
drawInfectionDates <- function(
  ...
  Fhat <- make_empirical_cdf(
      shape = c(shapeIncubation, shapeOnsetToCount),
  ...
  for (replicateNum in 1:numberOfReplicates) {
    ...
    for (dateTest in data_subset$date) {
      ...
      if (!is.na(incidenceOnDay) & incidenceOnDay > 0) {
        sampledInfectionToCountDelay <- round(
...
drawAllInfectionDates <- function(
  data,
  data_type = "confirmed",
  numberOfReplicates = 100,
  shapeIncubation,
      ...
      results_list <- lapply(
        ...
        function(x) {
          subset_data <- subset(data,
            region == x & source == source_i & data_type == count_type_i & variable == "incidence")

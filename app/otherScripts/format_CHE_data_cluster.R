...
for (newestFile in bagFiles) {
  ...
  fileDate <- strptime(
  stringr::str_match(newestFile, ".*\\/(\\d*-\\d*-\\d*_\\d*-\\d*-\\d*)")[, 2],
  format = "%Y-%m-%d_%H-%M-%S")

  maximum_file_date <- max(fileDate)
  ...
  ### Save file
  readr::write_csv(final_delay_data_FOPH,
    path = here::here("app", "data", "dev-clusterData","data", "delays",
      str_c(format(fileDate, "%Y%m%d"), "_CHE_data_delays.csv")
      )
  )
  ...
  testsFile <- str_replace(newestFile, "FOPH_COVID19_data_extract.csv", "Time_series_tests.csv")
  nTests <- read_delim(file = testsFile, delim = ";",
                      col_types = cols_only(
  ...
  readr::write_csv(
    allBAGdata,
    path = here::here("app", "data", "dev-clusterData", "data", "incidenceData",
      str_c(format(fileDate, "%Y%m%d"), "_incidence_data_CHE.csv")
      )
  )
}

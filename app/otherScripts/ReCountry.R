...
if (dim(countryData)[1] > 0) {
  ...
  ## Added loops for the loess options and 
  ## bootstrap
  AllcountryEstimates <- data.frame()
  for (days_incl in c(14, 21)){
    for (bootstrap_setting in c("legacy", "block_bootstrap")){
      ...
      # will override each time independent of days_incl / bootstrap_setting
      ...
      if (bootstrap_setting == "legacy"){
        countryEstimates <- cleanCountryReEstimate(countryEstimatesRaw, method = 'legacy') %>%
          left_join(
            dplyr::select(popData, region, countryIso3),
            by = c("region")
          ) %>% mutate(estimate_type = paste0(estimate_type, '_legacy'))
      } else {
      ...
      if (days_incl == 21){
        AllcountryEstimates <- bind_rows(AllcountryEstimates, 
                                         countryEstimates)
      } else {
        AllcountryEstimates <- bind_rows(AllcountryEstimates, 
                                      countryEstimates %>% mutate(estimate_type = paste0(days_incl, '_', estimate_type)) )
      }
      # End of loop across LOESS / Bootstrap setting
    }}
  countryEstimates <- AllcountryEstimates

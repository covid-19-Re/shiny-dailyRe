...
parseRKIdate <- function(x){as_date(parse_date_time(x, "%Y/%m/%d %H%M%S", tz = "UTC"))}

reformatted_DEU_data <- data_Germany %>%
  mutate(count_date = parseRKIdate(Meldedatum),
         reference_date = parseRKIdate(Refdatum),
         symptom_onset = IstErkrankungsbeginn) %>%
  filter(NeuerFall %in% c(0, 1)) %>%
  dplyr::select(c('reference_date', 'count_date', 'symptom_onset',
                  "AnzahlFall")) %>%
  uncount(AnzahlFall) %>% 
...
write_csv(delays_data_DEU, path = file.path(outDir, "DEU_data_delays.csv"))
...
confirmed_case_data <- first_curation_DEU_data %>%
  ...
  # mutate(data_type = "confirmed", #TODO remove commented section
  #        local_infection = "TRUE",
  #        date_type = "report",
  #        date = count_date) %>%

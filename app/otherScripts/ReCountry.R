...
argsRaw <- commandArgs(trailingOnly = TRUE)
# parse args
if (length(argsRaw) == 0) {
  args <- list("CHE", FALSE)
  warning(str_c("Testing mode!! Country: ", args[1]))
} else if (length(argsRaw) == 1) {
  args <- list(argsRaw, FALSE)
} else if (length(argsRaw) == 2) {
  args <- list(argsRaw[1], as.logical(argsRaw[2]))
} else {
...
if (args$regenerateTS) {
  cat(str_c("\n", Sys.time(), " | ", args$country, ": using whole timeseries ...\n"))
} else {
  cat(str_c("\n", Sys.time(), " | ", args$country, ": using truncated timeseries ...\n"))
}
